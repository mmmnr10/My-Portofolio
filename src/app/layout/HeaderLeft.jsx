...
export default function HeaderLeft({ darkMode, toggleDarkMode }) {
  ...
  return (
    <motion.div
      ...
      className="glass p-8 sticky top-8 rounded-[2rem] shadow-2xl overflow-hidden border border-white/5 transition-all duration-500 hover:border-[#f2a641]/20"
    >
      {/* Decorative Glow */}
      <div className="absolute -top-24 -left-24 w-48 h-48 bg-[#f2a641]/10 blur-[100px] rounded-full pointer-events-none" />

      <div className="flex flex-col gap-8 relative z-10">
        {/* Profile Section */}
        <div className="flex flex-col items-center gap-6 text-center">
          <div className="relative group w-full px-4">
            <div className="absolute inset-0 bg-gradient-to-tr from-[#d48b26] to-[#ffd875] rounded-[2.5rem] blur-xl opacity-20 group-hover:opacity-40 transition-opacity" />
            <motion.img
              ...
              src="/Mustaf.jpeg"
              alt="Mustaf portrait"
              className="relative w-full aspect-[4/5] rounded-[2.5rem] object-cover border border-white/20 dark:border-white/10 shadow-[0_0_30px_rgba(0,0,0,0.5)] floating"
            />
          </div>
          ...
          <div className="space-y-2">
            <h1 className="text-3xl font-outfit font-bold tracking-tight text-white">
              Mustaf <span className="text-[#f2a641]">A</span>
            </h1>
            <div className="flex flex-col items-center gap-2">
              <p className="px-4 py-1.5 bg-[#f2a641]/10 text-[#f2a641] rounded-full text-sm font-medium inline-block">
        ...
        <div className="space-y-4 pt-4 border-t border-black/5 dark:border-white/5">
          ...
          <div className="relative">
            <button
              ...
              className="w-full flex items-center gap-3 p-3 rounded-2xl hover:bg-white/5 hover:text-[#f2a641] transition-all text-gray-300 group"
            >
              <MdOutlineEmail className="text-2xl text-[#f2a641]" />
            ...
            <AnimatePresence>
              {showTooltipEmail && (
                <motion.span
                  ...
                  animate={{ opacity: 1, y: 0 }}
                  exit={{ opacity: 0 }}
                  className="absolute left-1/2 -translate-x-1/2 -top-10 bg-[#f2a641]/20 backdrop-blur-md border border-[#f2a641]/50 text-white text-xs px-3 py-1.5 rounded-lg whitespace-nowrap z-50 shadow-[0_0_20px_rgba(242,166,65,0.2)]"
                >
          ...
          <a href="tel:0762665179" className="flex items-center gap-3 p-3 rounded-2xl hover:bg-white/5 hover:text-[#f2a641] transition-all text-gray-300 group">
            <IoCallOutline className="text-2xl text-[#f2a641]" />
        ...
        <div className="space-y-4">
          ...
          <motion.a
            ...
            target="_blank"
            className="flex items-center gap-3 p-3 rounded-2xl bg-[#f2a641] text-black shadow-[0_0_20px_rgba(242,166,65,0.2)] hover:shadow-[0_0_40px_rgba(242,166,65,0.4)] transition-all group font-bold"
          >
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-6">
            {education.map((item, index) => (
              <div key={index} className="relative pl-6 before:absolute before:left-0 before:top-2 before:bottom-0 before:w-0.5 before:bg-gradient-to-b before:from-[#f2a641] before:to-transparent">
                ...
                <span className="text-[10px] font-bold text-[#f2a641] mt-1 block uppercase tracking-wider">{item.year}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="space-y-4 pb-4">
          ...
          <div className="flex flex-wrap gap-2">
            {hobbies.map((hobby, index) => (
              <span
                key={index}
                className="px-3 py-1.5 rounded-xl bg-white/5 border border-white/10 text-xs font-medium text-gray-300 hover:border-[#f2a641] transition-colors"
              >
                {hobby}
              </span>
            ))}
...
function ContactItem({ icon, text }) {
  return (
    <div className="flex items-center gap-3 p-3 rounded-2xl text-gray-300">
      <span className="text-2xl text-[#f2a641]">{icon}</span>
    ...
    <div className="space-y-1.5">
      ...
      <div className="h-1 w-full bg-white/10 rounded-full overflow-hidden">
        <motion.div
          initial={{ width: 0 }}
          animate={{ width: "100%" }}
          className={`h-full bg-gradient-to-r from-[#d48b26] to-[#ffd875] ${progress}`}
        />
      </div>
    </div>
  );
